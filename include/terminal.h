...
#define ALT_KEY(k) (char)(((unsigned char)(k) + 0x80))
 ...
 //The behavior of iscntrl and _iscntrl_l is undefined if c isn't EOF or in the range 0 through 0xFF, inclusive. 
 //When a debug CRT library is used and c isn't one of these values, the functions raise an assertion.
static int myiscntrl(int k)
{
    if ((k < 0) || (k > 255)) return 1;
    else return iscntrl(k);
}
