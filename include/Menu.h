...
class Menu{
    // Read the following comments above the variables and functions
    ...
    public:
        ...
        std::string key_field(std::string name);
...
std::string Menu::key_field(std::string name){
    std::string::size_type sz;
    sz = name.find_first_of(':');
...
void Menu::edit_field(std::string old_name, std::string new_name, std::function<void (void)> new_func){
    ...
    {
        if (key_field(names[itr].first) == old_name)
        {
...
bool Menu::has_field(std::string name){
    for(auto itr = names.begin(); itr != names.end(); itr++){
        if (key_field(itr->first) == name) return true;
...
void Menu::print_neutral_menu(){
    ...
    for(auto itr = names.begin(); itr != names.end(); itr++){
        std::cout << ((itr != pos)? "|": "|");
        gen_element(itr->first,colors.second);
        std::cout << ((itr != pos)? "|": "|") << std::endl;
    }
